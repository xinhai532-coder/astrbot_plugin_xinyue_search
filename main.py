...
@register("astrbot_plugin_xinyue_search", "阿立", "心悦搜索机器人插件", "1.3.7")
class XinyueSearchBotPlugin(Star):
    def __init__(self, context: Context, config: dict = None):
        ...
        # 尝试获取机器人QQ号
        config_qq = config.get('bot_qq', '') if config else ''  # 用户配置的QQ号
        actual_qq = ''  # 实际运行的QQ号
        ...
        if not license_token:
            ...
            try:
                validator = LicenseValidator(None)  # 试用模式
            ...
            try:
                ...
                if license_info:
                    logger.info(f"📋 授权用户: {license_info.get('user_info', '未知')}")
                    logger.info(f"📦 套餐类型: {license_info.get('plan_type', '未知')}")
                    logger.info(f"📅 到期时间: {license_info.get('expire_time', '未知')}")
    ...
    def _check_license(self):
        ...
        # 可选：定期重新验证（增加安全性）
        # if hasattr(self, '_license_validator'):
        #     # 这里可以添加定期验证逻辑
        #     pass
        else:
